...
export class DatosCvComponent implements OnInit {
	...
	uploadFile(event: any) {
		...
		if (this.file != undefined) {
			...
			this.sigaServices.postSendContent('cargaMasivaDatosCurriculares_uploadFile', this.file).subscribe(
				(data) => {
					this.file = null;
	...
	isBuscar() {
		...
		this.sigaServices.postPaginado('cargaMasivaDatosCurriculares_searchCV', '?numPagina=1', this.body).subscribe(
			(data) => {
				this.progressSpinner = false;
	...
	downloadFile(data: Response) {
		this.progressSpinner = true;
		this.sigaServices.postDownloadFiles('cargaMasivaDatosCurriculares_generateExcelCV', this.body).subscribe(
			(data) => {
	...
	downloadOriginalFile(selectedDatos) {
		...
		this.sigaServices.postDownloadFiles('cargaMasivaDatosCurriculares_downloadOriginalFile', this.body).subscribe(
			(data) => {
				const blob = new Blob([ data ], { type: 'text/csv' });
	...
	downloadLogFile(selectedDatos) {
		...
		this.sigaServices.postDownloadFiles('cargaMasivaDatosCurriculares_downloadLogFile', this.body).subscribe(
			(data) => {
				const blob = new Blob([ data ], { type: 'text/csv' });
