...
export class BusquedaCensoGeneralComponent implements OnInit {
	...
	isBuscar() {
		if (this.checkFilters()) {
			if (this.body.numeroColegiado != undefined && this.body.numeroColegiado != '') {
				if (this.colegios_seleccionados != undefined && this.colegios_seleccionados.length > 0) {
					this.search();
				} else {
					this.showFail(
						this.translateService.instant('censo.busquedaCensoGeneral.mensaje.introducir.colegio')
					);
				}
			} else {
				this.search();
	...
	isBuscarExacta() {
		if (this.checkFiltersExact()) {
			...
			this.sigaServices.postPaginado('busquedaCensoGeneral_searchExact', '?numPagina=1', this.body).subscribe(
				(data) => {
			...
			this.showFail(
				this.translateService.instant('censo.busquedaCensoGeneral.mensaje.introducir.numero.colegiado')
			);
		}
	}
	...
	search() {
		...
		this.sigaServices.postPaginado('busquedaCensoGeneral_search', '?numPagina=1', this.body).subscribe(
			(data) => {
				this.progressSpinner = false;
				this.bodySearch = JSON.parse(data['body']);
				this.datos = this.bodySearch.busquedaFisicaItems;
	...
	getColegiado(selectedDatos) {
		...
		this.sigaServices
			.postPaginado('busquedaCensoGeneral_searchColegiado', '?numPagina=1', this.bodyColegiado)
			.subscribe(
				(data) => {
					...
					if (
						this.datosColegiados == null ||
						this.datosColegiados == undefined ||
						this.datosColegiados.length == 0
					) {
						this.getNoColegiado(selectedDatos);
					} else {
						sessionStorage.setItem('personaBody', JSON.stringify(this.datosColegiados[0]));
						sessionStorage.setItem('esColegiado', JSON.stringify(true));
	...
	getNoColegiado(selectedDatos) {
		...
		this.sigaServices
			.postPaginado('busquedaNoColegiados_searchNoColegiado', '?numPagina=1', this.bodyNoColegiado)
			.subscribe(
				(data) => {
					this.progressSpinner = false;
					...
					if (this.datosNoColegiados.length > 0) {
						...
						sessionStorage.setItem('esColegiado', JSON.stringify(false));

						sessionStorage.setItem('personaBody', JSON.stringify(this.datosNoColegiados[0]));
	...
	getCliente(selectedDatos) {
		...
		this.sigaServices
			.postPaginado('busquedaCensoGeneral_searchCliente', '?numPagina=1', this.bodyNoColegiado)
			.subscribe(
				(data) => {
					this.progressSpinner = false;
					this.noColegiadoSearch = JSON.parse(data['body']);
	...
	disableBuscar(): boolean {
		if (
			...
			(this.body.primerApellido != undefined && this.body.primerApellido != '') ||
			(this.body.segundoApellido != undefined && this.body.segundoApellido != '') ||
			(this.body.numeroColegiado != undefined && this.body.numeroColegiado != '') ||
			(this.colegios_seleccionados != undefined && this.colegios_seleccionados.length > 0)
		) {
			return false;
		} else {
			return true;
	...
	checkFiltersExact() {
		if (this.body.nif == null || this.body.nif == undefined || this.body.nif.trim().length < 3) {
	...
	checkFilters() {
		if (
			(this.body.nombre == null || this.body.nombre == undefined || this.body.nombre.trim().length < 3) &&
