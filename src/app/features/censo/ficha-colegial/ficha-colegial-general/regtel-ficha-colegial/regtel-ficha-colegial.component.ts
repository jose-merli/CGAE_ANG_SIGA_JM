...
import { Component, OnInit, Input, ViewChild, OnChanges, ViewEncapsulation, SimpleChanges } from '@angular/core';
...
export class RegtelFichaColegialComponent implements OnInit {
	...
	ngOnChanges(changes: SimpleChanges) {
		...
		// 	this.sigaServices
		// 		.postPaginado('fichaColegialRegTel_searchListDoc', '?numPagina=1', this.idPersona)
		// 		.subscribe(
		// 			(data) => {
		...
		// 			}
		// 		);
		// } else if (this.tarjetaRegtel == '3' || this.tarjetaRegtel == '2') {
		// 	this.sigaServices
		// 		.postPaginado('fichaColegialRegTel_searchListDocNoCol', '?numPagina=1', this.idPersona)
		// 		.subscribe(
		// 			(data) => {
		// 				this.bodySearchRegTel = JSON.parse(data['body']);
		// 				this.bodyRegTel = this.bodySearchRegTel.docuShareObjectVO;
	...
	onClickAtrasRegtel() {
		...
		if (this.esColegiado) {
			this.sigaServices
				.postPaginado('fichaColegialRegTel_searchListDir', '?numPagina=1', selectedRegtel)
				.subscribe(
					(data) => {
						this.bodySearchRegTel = JSON.parse(data['body']);
						this.bodyRegTel = this.bodySearchRegTel.docuShareObjectVO;
			...
			this.sigaServices
				.postPaginado('fichaColegialRegTel_searchListDirNoCol', '?numPagina=1', selectedRegtel)
				.subscribe(
					(data) => {
						this.bodySearchRegTel = JSON.parse(data['body']);
						this.bodyRegTel = this.bodySearchRegTel.docuShareObjectVO;
	...
	addCollectionRegtel() {
		...
		this.sigaServices.post(url, this.generalBody.idPersona).subscribe(
			(data) => {
