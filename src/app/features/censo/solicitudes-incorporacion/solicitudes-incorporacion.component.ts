...
export class SolicitudesIncorporacionComponent implements OnInit {
	...
	buscarSolicitudes() {
		...
		this.sigaServices.postPaginado('solicitudIncorporacion_searchSolicitud', '?numPagina=1', this.body).subscribe(
			(result) => {
	...
	activarPaginacion() {
		if (!this.bodySearch.solIncorporacionItems || this.bodySearch.solIncorporacionItems.length == 0) return false;
		else return true;
	}
	isSelectMultiple() {
