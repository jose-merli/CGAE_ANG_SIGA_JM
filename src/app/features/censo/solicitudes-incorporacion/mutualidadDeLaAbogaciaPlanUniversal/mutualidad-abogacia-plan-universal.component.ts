...
export class MutualidadAbogaciaPlanUniversal implements OnInit {
	...
	ngOnInit() {
		...
		this.sigaServices.post('mutualidad_searchSolicitud', mutualidadRequest).subscribe(
			(result) => {
				let resultParsed = JSON.parse(result.body);
				if (resultParsed.idsolicitud != null && resultParsed.idsolicitud != undefined) {
					...
					this.sigaServices.post('mutualidad_estadoMutualista', this.solicitud).subscribe(
						(result) => {
		...
		this.sigaServices.post('mutualidad_estadoSolicitud', this.solicitud).subscribe(
			(result) => {
				let prueba = JSON.parse(result.body);
				this.solicitud.idSolicitudMutualidad = prueba.idSolicitud;
				this.solicitud.estadoMutualidad = prueba.valorRespuesta;
	...
	obtenerCuotaYCapObj() {
		...
		this.sigaServices.post('mutualidad_obtenerCuotaYCapObjetivo', this.solicitud).subscribe(
			(result) => {
				let prueba = JSON.parse(result.body);
	...
	cargarCombos() {
		this.sigaServices.post('mutualidad_getEnums', '').subscribe(
			(result) => {
				...
				if (prueba.asistenciaSanitaria != null) {
					...
					this.designacionBeneficiarios = prueba.designacionBeneficiarios.combooItems;
		...
		this.sigaServices.get('solicitudIncorporacion_pais').subscribe(
			(result) => {
				...
				this.progressSpinner = false;
				this.paisDesc = this.paises.find((item) => item.value === this.paisSelected);
			},
			(error) => {
				console.log(error);
			}
...
para poder filtrar el dato con o sin estos caracteres*/
	...
	onChangeProvincia(event) {
		this.sigaServices.getParam('direcciones_comboPoblacion', '?idProvincia=' + event.value.value).subscribe(
			(result) => {
	...
	guardar() {
		...
		this.sigaServices.post('mutualidad_solicitudPolizaProfesional', solicitud).subscribe(
			(result) => {
				let prueba = JSON.parse(result.body);
			},
			(error) => {
