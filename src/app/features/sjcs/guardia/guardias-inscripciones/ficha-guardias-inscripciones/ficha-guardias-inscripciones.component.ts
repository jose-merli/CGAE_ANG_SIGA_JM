...
export class FichaGuardiasInscripcionesComponent implements OnInit {
  ...
  constructor(public datepipe: DatePipe, private translateService: TranslateService, private route: ActivatedRoute, private sigaServices: SigaServices, private location: Location, private persistenceService: PersistenceService, private router: Router, private commonsService: CommonsService, private confirmationService: ConfirmationService, private localStorageService: SigaStorageService, private datePipe: DatePipe) {}
  ...
  ngOnInit() {
    ...
    this.commonsService
      .checkAcceso(procesos_oficio.tarjetaResumen)
      .then((respuesta) => {
    ...
    this.commonsService
      .checkAcceso(procesos_oficio.tarjetaResumenCola)
      .then((respuesta) => {
    ...
    if (this.idPersona == null) {
      ...
      this.sigaServices.post("busquedaColegiados_searchColegiado", colegiadoConectado).subscribe((data) => {
  ...
  llamadaBackValidar(objetoValidacion, estado) {
    ...
    if (estado == "6") {
      //validacion de estado Pendiente de Alta

      this.sigaServices.post("guardiasInscripciones_validarInscripciones", objetoValidacion).subscribe(
        (data) => {
      ...
      //validacion de estado Pendiente de Baja

      //comprobacion de si la inscripcion tiene trabajos en SJCS
      this.sigaServices.post("guardiasInscripciones_buscarTrabajosSJCS", objetoValidacion).subscribe(
        (data) => {
          //console.log("entra en el data");
          this.progressSpinner = false;
          ...
          if (this.existeTrabajosSJCS == "true") {
            ...
            this.sigaServices.post("guardiasInscripciones_buscarsaltoscompensaciones", objetoValidacion).subscribe(
              (data) => {
                this.existeSaltosCompensaciones = JSON.parse(data.body);

                if (this.existeSaltosCompensaciones == true) {
                  let mess = this.translateService.instant("justiciaGratuita.oficio.inscripciones.mensajeSaltos");
                  let icon = "fa fa-edit";
  ...
  llamadaBackSaltosCompensaciones() {
    this.progressSpinner = true;
    this.sigaServices.post("guardiasInscripciones_buscarsaltoscompensaciones", this.objetoValidacion).subscribe(
      (data) => {
        this.progressSpinner = false;
  ...
  solicitarBaja() {
    ...
    if (this.formatDate(this.datos.fechaActual) < this.formatDate(fechaHoy)) {
      ...
      this.sigaServices.post("guardiasInscripciones_buscarGuardiasAsocTurnos", this.objetoValidacion).subscribe();
  ...
  llamadaBackSolicitarBaja() {
    this.progressSpinner = true;
    this.sigaServices.post("guardiasInscripciones_solicitarBajaInscripciones", this.objetoValidacion).subscribe(
      (data) => {
        this.progressSpinner = false;
  ...
  llamadaBackTrabajosSJCS(accion) {
    this.progressSpinner = true;
    this.sigaServices.post("guardiasInscripciones_buscarTrabajosSJCS", this.objetoValidacion).subscribe(
      (data) => {
        this.progressSpinner = false;
  ...
  cambiarFecha() {
    ...
    if (resultComparacion != -1) {
      //  la seleccionada fechasolicitudNUEVA > fechaEfectivaAlta
  ...
  llamadaBackCambiarFecha() {
    this.progressSpinner = true;
    this.sigaServices.post("guardiasInscripciones_cambiarFechaInscripciones", this.objetoValidacion).subscribe(
      (data) => {
        this.progressSpinner = false;
    ...
    if (this.inscripcionesSelected.inscripcionesSelected != undefined) {
      ...
      this.sigaServices.post("guardiasInscripciones_insertSolicitarAlta", body).subscribe(
        (data) => {
          ...
          this.showMessage("success", this.translateService.instant("general.message.correct"), this.translateService.instant("general.message.accion.realizada") + ". Se ha dado de alta correctamente");
          this.progressSpinner = false;
