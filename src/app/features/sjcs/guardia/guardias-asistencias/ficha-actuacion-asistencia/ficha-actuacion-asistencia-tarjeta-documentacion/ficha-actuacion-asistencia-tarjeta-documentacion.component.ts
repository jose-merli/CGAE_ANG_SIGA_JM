...
@Component({
  selector: 'app-ficha-actuacion-asistencia-tarjeta-documentacion',
  templateUrl: './ficha-actuacion-asistencia-tarjeta-documentacion.component.html',
  styleUrls: ['./ficha-actuacion-asistencia-tarjeta-documentacion.component.scss']
})
export class FichaActuacionAsistenciaTarjetaDocumentacionComponent implements OnInit {
      ...
      this.sigaServices.postSendFileAndIdAsistencia("busquedaGuardias_subirDocumentoAsistencia", this.documentaciones, this.idAsistencia).subscribe(
        n => {
          ...
          if(result.error){
            ...
            this.getDocumentacion();
            this.refreshTarjetas.emit(this.idActuacion);
          }
        },
        err => {
          console.log(err);
