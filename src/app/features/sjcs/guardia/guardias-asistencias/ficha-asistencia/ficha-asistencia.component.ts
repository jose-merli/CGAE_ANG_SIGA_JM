...
export class FichaAsistenciaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private location: Location, private translateService: TranslateService, private sigaServices: SigaServices, private router: Router, private datePipe: DatePipe, private commonServices: CommonsService, private persistenceService: PersistenceService) {}
  ...
  async ngOnInit() {
    ...
    this.commonServices
      .checkAcceso(procesos_guardia.tarjeta_caracteristicas_asistencias)
      .then((respuesta) => {
    ...
    //Facturaciones
    this.commonServices
      .checkAcceso(procesos_facturacionSJCS.tarjetaFacFenerica)
      .then((respuesta) => {
  ...
  initTarjetas() {
    ...
    this.listaTarjetas.forEach((tarj) => {
      ...
      if (tarj.nombre != "Datos Generales") {
        //Durante la creacion, deshabilitamos las tarjetas que no sean la de datos generales
        tarj.detalle = false;
      }
    });
    ...
    this.sigaServices.getParam("busquedaGuardias_buscarTarjetaAsistencia", "?anioNumero=" + event).subscribe(
      (n) => {
        ...
        if (n.error !== null && n.error.code === 500) {
          ...
          let camposContrarios = [];
          if (
            !newAsistenciaData.numContrarios || //Si esta vacio
            (newAsistenciaData.numContrarios && newAsistenciaData.numContrarios == "0")
          ) {
            //O si viene relleno y vale 0
          ...
          } else if (newAsistenciaData.incidencias == null) {
            camposObservaciones = [
              {
          ...
          this.listaTarjetas.forEach((tarj) => {
            if (tarj.id != "datosgenerales" && tarj.id != "caracteristicas") {
              //Una vez creada la asistencia, dejamos abrir las demas tarjetas
              tarj.detalle = true;
            }
          });
  ...
  backTo() {
    if (sessionStorage.getItem("vieneDeFichaDesigna")) {
      ...
      if (sessionStorage.getItem("modoBusqueda")) {
        searchMode = sessionStorage.getItem("modoBusqueda");
      }
