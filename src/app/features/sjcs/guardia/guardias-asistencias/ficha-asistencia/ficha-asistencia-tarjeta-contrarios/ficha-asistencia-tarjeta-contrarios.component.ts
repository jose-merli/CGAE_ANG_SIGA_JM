...
export class FichaAsistenciaTarjetaContrariosComponent implements OnInit {
  ...
  constructor(private translateService: TranslateService, private changeDetectorRef: ChangeDetectorRef, private sigaServices: SigaServices, private router: Router, private persistenceService: PersistenceService, private commonService: CommonsService) {}
  ...
  onSelectRow(contrario: ContrarioItem) {
    if (this.table.selectionMode == "single") {
      ...
      if (this.mostrarHistorico && contrario.fechaBaja && this.numSeleccionado > 0) {
  ...
  onClickEnlace(contrario: ContrarioItem) {
    //Redirigimos a ficha de justiciables
    let datos;
  ...
  deleteContrarios() {
    ...
    if (Array.isArray(this.selectedDatos)) {
      //Si hemos seleccionado varios registros o hemos seleccionado al menos uno
      contrarios = this.selectedDatos;
    } else {
      contrarios.push(this.selectedDatos);
    }

    this.sigaServices.postPaginado("busquedaGuardias_desasociarContrario", "?anioNumero=" + this.idAsistencia, contrarios).subscribe(
      (data) => {
  ...
  reactivarContrario() {
    ...
    this.sigaServices.postPaginado("busquedaGuardias_asociarContrario", "?anioNumero=" + this.idAsistencia, justiciablesContrarios).subscribe(
      (data) => {
        let result = JSON.parse(data["body"]);
        if (result.error) {
          this.showMsg("error", this.translateService.instant("justiciaGratuita.guardia.asistenciasexpress.errorguardar"), result.error.description);
  ...
  actualizaSeleccionados() {
    if (this.table.selectionMode == "single") {
      ...
      this.numSeleccionado = this.selectedDatos.length;
      if (this.mostrarHistorico && this.numSeleccionado <= 0) {
        this.reactivarDisabled = true;
      }
      if (this.numSeleccionado <= 0) {
        this.deleteDisabled = true;
