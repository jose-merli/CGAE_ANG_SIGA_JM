...
export class FichaAsistenciaTarjetaAsistidoComponent implements OnInit {
  ...
  constructor(private commonServices: CommonsService, private sigaServices: SigaServices, private translate: TranslateService, private router: Router, private persistenceService: PersistenceService, private confirmationService: ConfirmationService, private translateService: TranslateService) {}
  ...
  asociarJusticiable(actualiza: boolean) {
    ...
    this.sigaServices.postPaginado("busquedaGuardias_asociarAsistido", "?anioNumero=" + this.idAsistencia + "&actualizaDatos=" + actualizaDatos, this.asistido).subscribe(
      (data) => {
  ...
  desasociarJusticiable(desasocia: boolean) {
    if (desasocia) {
      ...
      this.sigaServices.postPaginado("busquedaGuardias_desasociarAsistido", "?anioNumero=" + this.idAsistencia, this.asistido).subscribe(
        (data) => {
          let result = JSON.parse(data["body"]);
          if (result.error) {
            this.showMsg("error", this.translate.instant("justiciaGratuita.guardia.asistenciasexpress.errorguardar"), result.error.description);
