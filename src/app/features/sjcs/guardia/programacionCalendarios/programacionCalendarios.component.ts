...
export class ProgramacionCalendariosComponent implements OnInit {
  ...
  constructor(private persistenceService: PersistenceService, private sigaServices: SigaServices, private commonsService: CommonsService, private translateService: TranslateService, private router: Router, public oldSigaServices: OldSigaServices, private confirmationService: ConfirmationService, private trmService: TablaResultadoMixIncompService, private authenticationService: AuthenticationService, private datepipe: DatePipe) {
  ...
  ngOnInit() {
    ...
    this.commonsService
      .checkAcceso(procesos_guardia.guardias)
      .then((respuesta) => {
...
rowToDelete.cells.forEach((c, index) => {
  ...
  delete(indexToDelete) {
    ...
    toDelete.forEach((row) => {
      ...
      let deleteParams = { idTurno: idTurno, idGuardia: idGuardia, idCalendarioProgramado: idCalendarioProgramado, idInstitucion: idInstitucion, fechaDesde: fechaDesde, fechaHasta: fechaHasta, facturado: facturado, asociadoAsistencias: asociadoAsistencias, contadorGenerados: contadorGenerado };
  ...
  eliminarCal(lista) {
    this.progressSpinner = true;
    this.sigaServices.post("guardiaCalendario_eliminar", lista).subscribe(
      (data) => {
        let error = JSON.parse(data.body).error;
  ...
  guardarInc(nombreTurno, nombreGuardia, nombreTurnoIncompatible, nombreGuardiaIncompatible, idTurnoIncompatible, idGuardiaIncompatible, motivos, diasSeparacionGuardias, idGuardia, idTurno) {
    ...
    this.sigaServices.post("guardiasIncompatibilidades_guardarIncompatibilidades", this.saveIncompatibilidadesDatosEntradaItem).subscribe(
      (data) => {
        let error = JSON.parse(data.body).error;
