...
export class FichaProgramacionComponent implements OnInit {
  ...
  constructor(private persistenceService: PersistenceService, private location: Location, private sigaServices: SigaServices, private commonService: CommonsService, private translateService: TranslateService, private router: Router, private datepipe: DatePipe, private globalGuardiasService: GlobalGuardiasService) {}
  ...
  ngOnInit() {
    ...
    if (sessionStorage.getItem("filtrosBusquedaGuardiasFichaGuardia")) {
      ...
      this.persistenciaGuardia = JSON.parse(sessionStorage.getItem("filtrosBusquedaGuardiasFichaGuardia"));
    }
    ...
    if (sessionStorage.getItem("mensaje")) {
      ...
      if (message) this.showMessage(message.severity, message.summary, message.detail);
  ...
  obtenerPermisos() {
    ...
    this.progressSpinner = true;
    this.commonService
      .checkAcceso(procesos_guardia.resumen)
      .then((respuesta) => {
    ...
    this.commonService
      .checkAcceso(procesos_guardia.turno)
      .then((respuesta) => {
    ...
    this.commonService
      .checkAcceso(procesos_guardia.conf_cola)
      .then((respuesta) => {
    ...
    this.commonService
      .checkAcceso(procesos_guardia.conf_calendario)
      .then((respuesta) => {
    ...
    this.commonService
      .checkAcceso(procesos_guardia.incompatibilidades)
      .then((respuesta) => {
    ...
    this.commonService
      .checkAcceso(procesos_guardia.calendario)
      .then((respuesta) => {
    ...
    this.commonService
      .checkAcceso(procesos_guardia.datos_generales)
      .then((respuesta) => {
  ...
  searchGuardiasFromCal(event): Promise<any> {
    ...
    if (event.idCal != "") return this.getGuardiasFromCal(event.idCal, event.fechaDesde, event.fechaHasta);
  }
  ...
  getGuardiasFromCal(idCalendarioProgramado, fechaDesde, fechaHasta): Promise<any> {
    ...
    return this.sigaServices
      .getParam("guardiaCalendario_getGuardiasFromCalendar", `?idCalendar=${idCalendarioProgramado}&fechaDesde=${fechaDesde}&fechaHasta=${fechaHasta}`)
      .toPromise()
      .then(
        (data) => {
  ...
  getFechasProgramacion(idGuardia, dataToSave): Promise<any> {
    ...
    return this.sigaServices
      .getParam("guardiaCalendario_getFechasProgFromGuardia", "?idGuardia=" + idGuardia)
      .toPromise()
      .then(
        (data) => {
          fechasArr = data;
  ...
  updateCalendarData(datos) {
    this.progressSpinner = true;
    this.sigaServices.post("guardiaCalendario_updateCalendarioProgramado", datos).subscribe(
      (data) => {
        ...
        sessionStorage.setItem(
          ...
          JSON.stringify({
            severity: "info",
            summary: "Se ha actualizado correctamente",
  ...
  newCalendarProg(datos) {
    ...
    this.sigaServices.post("guardiaCalendario_newCalendarioProgramado", datos).subscribe(
      (data) => {
        //console.log('this.persistenciaGuardia: ', this.persistenciaGuardia)

        if (this.persistenciaGuardia != undefined) {
          sessionStorage.setItem("filtrosBusquedaGuardiasFichaGuardia", JSON.stringify(this.persistenciaGuardia));
        }
        ...
        if (datos.idCalG == null && (!datos.guardias || datos.guardias.length == 0)) {
          ...
          sessionStorage.setItem(
            ...
            JSON.stringify({
              severity: "info",
              summary: "Se ha creado correctamente",
      ...
      (err) => {
        ...
        if (error.error.message == "messages.factSJCS.error.solapamientoRango") {
          this.showMessage("error", this.translateService.instant("general.message.incorrect"), this.translateService.instant(error.error.message));
    ...
    if (event) {
      ...
      event.forEach((elem) => {
        let dato = {
          ...
          listaGuardias: this.datosGenerales.listaGuarias.label,
          idCalendarioProgramado: this.datosGenerales.idCalendarioProgramado,
          idCalendarioGuardias: elem.idCalendarioGuardias,
        };
        dataToDownload.push(dato);
      });

      this.sigaServices.getDownloadFiles("guardiaCalendario_descargarExcelLog", dataToDownload).subscribe(
        (resp) => {
          this.progressSpinner = false;
  ...
  generarCalendario(datos) {
    this.progressSpinner = true;
    this.sigaServices.post("guardiaCalendario_generar", datos).subscribe(
      (data) => {
        this.progressSpinner = false;
  ...
  generateModal() {
    if (this.controlGenerate()) this.displayProgramacion = true;
  }
