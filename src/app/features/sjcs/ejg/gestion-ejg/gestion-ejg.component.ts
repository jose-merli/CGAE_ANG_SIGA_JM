...
export class GestionEjgComponent implements OnInit {
  ...
  constructor(private sigaServices: SigaServices, private translateService: TranslateService, private persistenceService: PersistenceService, private router: Router, private location: Location, private commonsService: CommonsService, private datePipe: DatePipe) {
    ...
    if (this.persistenceService.getDatosEJG()) {
      ...
      if (sessionStorage.getItem("Designacion")) {
        ...
        if (this.tipoObject.idTurno) {
          ...
          this.sigaServices.getParam("componenteGeneralJG_tipoTurno", "?idTurno=" + this.tipoObject.idTurno).subscribe((idTipoTurno) => {
      ...
      this.sigaServices.post("busquedaPerJuridica_parametroColegio", paramTipoEJGColegio).subscribe((data) => {
        if (data != null && data != undefined) {
          const defaultTipoEJGColegio = JSON.parse(data.body).parametro;
          if (defaultTipoEJGColegio != undefined && defaultTipoEJGColegio.trim().length > 0 && !isNaN(defaultTipoEJGColegio) && Number(defaultTipoEJGColegio) > 0){
            this.body.tipoEJGColegio = JSON.parse(data.body).parametro;
          } 
        }
      });
      let paramTipoEJG = { valor: "codigo.general.scstipoejg.ordinarios" };
      this.sigaServices.post("busquedaPerJuridica_property", paramTipoEJG).subscribe((data) => {
        if (data != null && data != undefined) {
  ...
  private updateTarjResumen() {
    this.datosResumen = [
      ...
      { label: "CAJG", value: this.body.annioCAJG != null ? this.body.annioCAJG + "/" + this.body.numCAJG : "" },
