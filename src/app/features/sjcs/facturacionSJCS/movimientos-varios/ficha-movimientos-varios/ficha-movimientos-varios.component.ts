...
export class FichaMovimientosVariosComponent implements OnInit {
  ...
  async cargaInicial() {
    this.commonsService
      .checkAcceso(procesos_facturacionSJCS.busquedaMovimientosVarios)
      .then((respuesta) => {
    ...
    if (sessionStorage.getItem("datosNuevoMovimiento")) {
      ...
      const letrado = await this.getLetrado(datos.ncolegiado, this.sigaStorageService.institucionActual)
        .then((data) => {
    ...
    if (sessionStorage.getItem("datosEdicionMovimiento")) {
      ...
      this.movVarioDesdeTarjFacGeneEdit = await this.getMovimientoVarioPorId(datos.idObjeto)
        .then((data: MovimientosVariosFacturacionDTO) => {
      ...
      (n) => {
        ...
        this.datosListadoPagos.map((pago) => {
          if (this.datos.cantidad >= 0) {
            if (pago.cantidad < 0) cantidad = cantidad + pago.cantidad;
            else cantidad = cantidad - pago.cantidad;
  ...
  getDatosTarjetaResumen(movimiento: any) {
    ...
    datosResumen[3] = { label: "Importe: ", value: movimiento.cantidad.toString() + " €" };
  ...
  guardar() {
    ...
    if (this.datosClientes != null || this.datosClientes != undefined) {
      ...
      if (this.datosGenerales.descripcion == null || this.datosGenerales.descripcion == undefined || this.datosGenerales.descripcion == "" || this.datosGenerales.cantidad == null || this.datosGenerales.cantidad == undefined || this.datosGenerales.cantidad == 0) {
