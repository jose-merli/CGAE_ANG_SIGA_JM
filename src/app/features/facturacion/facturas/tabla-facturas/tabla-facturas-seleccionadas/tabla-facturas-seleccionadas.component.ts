...
@Component({
  selector: "app-tabla-facturas-seleccionadas",
  templateUrl: "./tabla-facturas-seleccionadas.component.html",
  styleUrls: ["./tabla-facturas-seleccionadas.component.scss"],
})
export class TablaFacturasSeleccionadasComponent implements OnInit, OnChanges {
  msgs: Message[] = [];
  progressSpinner: boolean = false;
  @Input() bodyInicial: FacturasItem[];
  itemsGeneral: FacturasItem[] = [];
      ...
      */
      ...
      this.grupos.forEach((element) => {
        ...
        grupitos.forEach((grup) => {
          item.estado = grup.estado;
          if (grup.importefacturado != null) {
            item.total += Number(grup.importefacturado);
            this.total += Number(grup.importefacturado);
          }
          this.totalPendiente += Number(grup.importeAdeudadoPendiente);
          item.formaPago = grup.nombreFormaPago;
          item.totalPendiente += Number(grup.importeAdeudadoPendiente);
          item.numFacturas++;
        });
