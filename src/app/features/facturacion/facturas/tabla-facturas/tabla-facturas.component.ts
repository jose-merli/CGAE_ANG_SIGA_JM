...
export class TablaFacturasComponent implements OnInit {
  ...
  constructor(private changeDetectorRef: ChangeDetectorRef, private sigaServices: SigaServices, private translateService: TranslateService, private confirmationService: ConfirmationService, private router: Router, private commonsService: CommonsService, private localStorageService: SigaStorageService, private persistenceService: PersistenceService) {}
  ...
  esUnaSociedad(filtros, selectedRow) {
    this.sigaServices
      .postPaginado("busquedaPerJuridica_datosGeneralesSearch", "?numPagina=1", filtros)
      .toPromise()
      .then(
        (n) => {
          ...
          sessionStorage.setItem("privilegios", JSON.stringify(true));
  ...
  noEsColegiado(filtros, selectedRow) {
    this.sigaServices
      .postPaginado("busquedaNoColegiados_searchNoColegiado", "?numPagina=1", filtros)
      .toPromise()
      .then(
        (n) => {
          let results = JSON.parse(n.body);
  ...
  esUnColegiado(filtros, selectedRow) {
    this.sigaServices
      .postPaginado("busquedaColegiados_searchColegiadoFicha", "?numPagina=1", filtros)
      .toPromise()
      .then(
        (n) => {
  ...
  fillFecha(event, campo) {
    if (campo === "fechaMin") this.itemAction.fechaMin = event;
  }
  ...
  eliminarUltimoCobro() {
    ...
    this.selectedDatos.forEach((element) => {
      if (element.idAccionUlt == "4") {
        ...
        (itemFacturaEstadosPagoItem.idFactura = element.idFactura), (itemFacturaEstadosPagoItem.idAccion = element.idAccionUlt), estadosPagosItems.push(itemFacturaEstadosPagoItem);
      }
    });
    ...
    if (estadosPagosItems.length > 0) {
      ...
      this.progressSpinner = true;
      this.sigaServices
        .post("facturacionPyS_eliminarEstadosPagos", estadosPagosItems)
        .toPromise()
        .then(
          (n) => {
  ...
  mensajeCargaMasiva(registrosProcesados, totalRegistros) {
    if (registrosProcesados != "0") {
      this.showMessage("success", this.translateService.instant("general.message.informacion"), this.translateService.instant("facturacion.facturas.cargaMasiva.procesamiento") + " " + registrosProcesados + "/" + totalRegistros + this.translateService.instant("consultas.consultaslistas.literal.registros"));
    } else {
      this.showMessage("error", this.translateService.instant("general.message.informacion"), this.translateService.instant("facturacion.facturas.cargaMasiva.procesamiento.error"));
    }
  }
  ...
  guardarDefi() {
    if (this.isValid() && this.requisitos()) {
      this.progressSpinner = true;
      this.sigaServices
        .post("facturacionPyS_insertarEstadosPagosVarios", this.itemsParaModificar)
        .toPromise()
        .then(
          (n) => {
