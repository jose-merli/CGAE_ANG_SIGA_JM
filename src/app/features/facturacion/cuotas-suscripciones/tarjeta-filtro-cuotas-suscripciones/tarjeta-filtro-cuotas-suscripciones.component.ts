...
export class TarjetaFiltroCuotasSuscripcionesComponent implements OnInit {
  ...
  constructor(private translateService: TranslateService, private sigaServices: SigaServices, private router: Router, private commonsService: CommonsService, private sigaStorageService: SigaStorageService, private persistenceService: PersistenceService) {}
  ...
  getComboCategoria() {
    this.subscriptionCategorySelectValues = this.sigaServices.get("tiposServicios_comboServicios").subscribe((CategorySelectValues) => {
  ...
  getComboTipo() {
    this.subscriptionTypeSelectValues = this.sigaServices.getParam("serviciosBusqueda_comboTiposMultiple", "?idCategoria=" + this.filtrosSuscripciones.idCategoria.toString()).subscribe((TipoSelectValues) => {
