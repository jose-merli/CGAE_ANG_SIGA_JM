...
export class HomeComponent implements OnInit {
  ...
  constructor(private sigaServices: SigaServices, private oldSigaServices: OldSigaServices, private authenticationService: AuthenticationService, private http: HttpClient, private localStorageService: SigaStorageService, private router: Router) {}

  ngOnInit() {
    this.removeUnusedSessionStorageKeys();
    this.sigaServices.get("getLetrado").subscribe((data) => {
      if (data.value == "S") {
  ...
  removeUnusedSessionStorageKeys() {
    let keysPorDefecto = ["Authorization", "tipoLogin", "AuthOldSIGA", "authenticated", "isLetrado", "esColegiado", "osAutenticated", "esNuevoNoColegiado", "personaBody"];
  ...
  getColegiadoLogeado() {
    ...
    this.sigaServices.postPaginado("busquedaColegiados_searchColegiadoFicha", "?numPagina=1", this.generalBody).subscribe((data) => {
