...
import { Cell, Row } from "./tabla-resultado-order-cg.service";
...
export class TablaResultadoOrderComponent implements OnInit {
  ...
  constructor(private renderer: Renderer2, private sigaServices: SigaServices, private commonServices: CommonsService, private cd: ChangeDetectorRef, private router: Router, private translateService: TranslateService, private sanitizer: DomSanitizer, private persistenceService: PersistenceService, private sigaStorageService: SigaStorageService) {
    ...
    if (this.rowGroups != undefined) {
      this.sigaServices.get("busquedaGuardia_turno").subscribe(
        (n) => {
          ...
          this.sigaServices.getParam("busquedaGuardia_guardia", "?idTurno=" + null).subscribe(
            (data) => {
              this.progressSpinner = false;
  ...
  selectRow(rowId, rowSelected) {
    ...
    if (this.selectedElement.includes(rowSelected)) {
      const i = this.selectedElement.indexOf(rowSelected);
      this.selectedElement.splice(i, 1);
    } else {
      this.selectedElement.push(rowSelected);
    }
        ...
        this.sigaServices.getParam("busquedaGuardia_guardia", "?idTurno=" + null).subscribe(
          (data) => {
            ...
            if (newRowGroups.length == 0) {
              ...
              if (this.checkOrdern()) this.guardarGuardiasEnConjunto.emit(event);
            } else {
              event = {
                newRowGroups: newRowGroups,
                update: false,
    ...
    if (this.calendarios) {
      ...
      if (groupSelected == null) {
        ...
        if (movement == "up") {
          ...
          //Sacamos su index
          let indexGrupoActual = this.rowGroups
            .map((row, index) => {
              if (row == ultimaRowDelGrupoActual) return { row, index };
            })
            .filter((row) => row)[0];
          ...
          this.rowGroups = this.swapElements(this.rowGroups, this.rowGroups.indexOf(actual[0]), this.rowGroups.indexOf(actual[actual.length - 1]) + 1, this.rowGroups.indexOf(aDesplazar[0]), this.rowGroups.indexOf(aDesplazar[aDesplazar.length - 1]) + 1);
  ...
  searchChange(j: any) {
    ...
    let isReturnArr = [];
    this.rowGroups = this.rowGroupsAux.filter((row) => {
      if (this.searchText[j] != " " && this.searchText[j] != undefined && !row.cells[j].value.toString().toLowerCase().includes(this.searchText[j].toLowerCase())) {
        isReturn = false;
      } else {
        isReturn = true;
      }
  ...
  onChangeGuardia(row: Row) {
    if (this.listaGuardias) {
      ...
      if (idTurno && idGuardia) {
        this.progressSpinner = true;
        this.sigaServices.getParam("listasGuardias_searchTipoDiaGuardia", "?idTurno=" + idTurno + "&idGuardia=" + idGuardia).subscribe(
          (data) => {
  ...
  getComboGuardia(idTurno, row) {
    this.progressSpinner = true;
    if (this.lista != undefined) {
      this.sigaServices.getParam("busquedaGuardia_guardiaNoBajaNoExistentesEnListaGuardias", "?idTurno=" + idTurno + "&idListaGuardias=" + this.lista.idLista).subscribe(
        (data) => {
          this.progressSpinner = false;
          let comboGuardia = data.combooItems;
          this.commonServices.arregloTildesCombo(comboGuardia);
      ...
      this.sigaServices.getParam("busquedaGuardia_guardia", "?idTurno=" + idTurno).subscribe(
        (data) => {
          this.progressSpinner = false;
          let comboGuardia = data.combooItems;
          this.commonServices.arregloTildesCombo(comboGuardia);
  ...
  descargarLog() {
    if (this.estado != "4" || this.estado != "2") {
      ...
      this.selectedElement.forEach((elem) => {
        let dato = {
          idCalendarioGuardias: elem.cells[7].value,
          idTurno: elem.cells[6].value,
          idGuardia: elem.cells[5].value,
        };
        datosSeleccionado.push(dato);
      });

      this.descargaLog.emit(datosSeleccionado);
    } else {
  ...
  openTab(row) {
    ...
    sessionStorage.setItem("itemFichaProgramacionCalendarios", JSON.stringify(this.tarjetaDatosGenerales));
    sessionStorage.setItem("filtrosBusquedaGuardiasFichaGuardia", JSON.stringify(guardiaItem));
    //this.persistenceService.setHistorico(evento.fechabaja ? true : false);
    this.router.navigate(["/gestionGuardias"]); // ficha guardias
  }

  openTab2(row) {
    let itemGuardiaColegiado = { guardia: row.cells[5].value, turno: row.cells[6].value, fechaDesde: this.tarjetaDatosGenerales.fechaDesde, fechaHasta: this.tarjetaDatosGenerales.fechaHasta };
  ...
  nuevoSaltoComp() {
    ...
    sessionStorage.setItem("itemSaltosCompColaGuardia", JSON.stringify(dataFilterFromColaGuardia));
    //this.persistenceService.setHistorico(evento.fechabaja ? true : false);
  ...
  getConfColaGuardias() {
    ...
    if (datos != null && datos.idTurno != null && datos.idGuardia != null) {
      this.sigaServices.post("busquedaGuardias_resumenConfCola", datos).subscribe(
        (data) => {
